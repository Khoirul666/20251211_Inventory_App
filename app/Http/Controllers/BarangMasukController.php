...
class BarangMasukController extends Controller
{
    public function barang_masuk()
    {
        ...
        $supplier = Supplier::find(Session::get('selected_supplier'));
        return view('barang_masuk.pilih_barang', compact('supplier'));
    }

    public function set_supplier(Request $request)
    {
        Session::put('selected_supplier', $request->id_supplier);
        return redirect('barang_masuk/pilih_barang');
    }
    ...
    {
        $cart = Session::get('cart_barang_masuk', []);
    ...
    public function list_barang_update($id, Request $request)
    {
        ...
        if (isset($cart[$id])) {
            ...
            $cart[$id]['total'] = $cart[$id]['harga_beli'] * $request->jumlah;

            Session::put('cart_barang_masuk', $cart);
    ...
    public function pilih_barang_store(Request $request)
    {
        ...
        $cart[$barang->id_barang] = [
            ...
            'total' => $request->jumlah * $barang->harga_beli,
        ];
        Session::put('cart_barang_masuk', $cart);
    ...
    public function forget_supplier()
    {
        Session::forget('selected_supplier');
        Session::forget('cart_barang_masuk');
    ...
    {
        $data = [
            'data_barang' => Session::get('cart_barang_masuk'),
            'data_user' => Session::get('selected_supplier')
        ];
        return $data;
    }
    ...
    public function getcheckout()
    {
        // return "aaa";
        $cart = Session::get('cart_barang_masuk');
        $id_cs = Session::get('selected_supplier');
        // dd($cart, $id_cs);
        if (empty($cart) || !$id_cs) {
            ...
            DB::transaction(function () use ($cart, $id_cs) {
                ...
                foreach ($cart as $cart_temp) {
                    ...
                    BarangMasuk::create([
                        ...
                        'id_invoicepembelian' => $inv_penjualan->id_invoicepembelian,
                        'tgl_masuk' => $inv_penjualan->tgl_cetak,
                        'nama_barang' => $data_barang->nama_barang,
                        'jumlah' => $cart_temp['jumlah_beli'],
                        'harga_beli' => $cart_temp['harga_beli'],
                    ]);
                    $data_barang->update([
                        'jumlah' => $data_barang->jumlah + $cart_temp['jumlah_beli'],
                    ]);
                    $count_total += $cart_temp['jumlah_beli'] * $cart_temp['harga_beli'];
                }
